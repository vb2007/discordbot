...
const { embedReplyPrimaryColorWithFieldsAndAuthor, embedReplyFailureColor } = require("../../helpers/embeds/embed-reply");
		...
		if(!interaction.inGuild()) {
			...
			var embedReply = embedReplyPrimaryColorWithFieldsAndAuthor(
				...
				[
					...
					{ name: "Categories", value: interaction.guild.channels.cache.filter((c) => c.type === 4).toJSON().length, inline: true },
					//size - 1 --> @everyone excluded
					{ name: "Roles (@everyone included)", value: interaction.guild.roles.cache.size, inline: true },
