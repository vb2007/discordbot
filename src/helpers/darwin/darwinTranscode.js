  ...
  return new Promise((resolve, reject) => {
    ffmpeg.ffprobe(inputPath, (err, metadata) => {
      ...
      resolve({
        ...
        rotation:
          videoStream.tags && videoStream.tags.rotate ? parseInt(videoStream.tags.rotate) : 0,
...
const calculateOptimalDimensions = (videoInfo) => {
  ...
  console.log(
    `Original dimensions: ${effectiveWidth}x${effectiveHeight}, aspect ratio: ${aspectRatio.toFixed(2)}`
  );
  if (needsScaling) {
...
export const transcodeVideo = async (inputPath, outputPath) => {
  try {
    ...
    console.log(
      `Original video: ${videoInfo.width}x${videoInfo.height}, rotation: ${videoInfo.rotation}°, codec: ${videoInfo.codec_name}`
    );
