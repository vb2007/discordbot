...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("time")
        .setDescription(
          "Specify the time in seconds (s), minutes (m), hours (h), or days (d). For example: 12m"
        )
        .setRequired(true)
    )
    .addStringOption((option) =>
  ...
  async execute(interaction) {
    ...
    if (!interaction.guild.members.me.permissions.has(PermissionFlagsBits.MuteMembers)) {
      ...
      var embedReply = embedReplyFailureColor("Timeout - Error", time, interaction);
    } else {
      try {
        ...
        var embedReply = embedReplySuccessColor("Timeout - Success", replyContent, interaction);
      } catch (error) {
        console.error(error);
        var embedReply = embedReplyFailureColor(
          "Timeout - Error",
