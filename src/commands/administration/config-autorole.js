...
module.exports = {
  data: new SlashCommandBuilder()
    .setName(commandName)
    .setDescription(
      "Sets a role to be automatically assigned to new members on join.",
    )
    ...
    .addRoleOption((option) =>
      option
        .setName("role")
        .setDescription(
          "Choose a role that will get assigned to the new server members.",
        )
        .setRequired(false),
    )
  ...
  async execute(interaction) {
    ...
    if (action === "configure") {
      try {
        ...
        if (!targetRole) {
          title = "AutoRole Configure: Error";
          description =
            "You must specify a role when configuring the autorole feature. :x:";
          return await replyAndLog(
            interaction,
            embedReplyFailureColor(title, description, interaction),
          );
    ...
    if (action === "disable") {
      try {
        ...
        const query = await db.query(
          "SELECT guildId FROM configAutorole WHERE guildId = ?",
          [currentGuildId],
        );
        const autoroleGuildId = query[0]?.guildId || null;

        if (autoroleGuildId) {
          await db.query("DELETE FROM configAutorole WHERE guildId = ?", [
            autoroleGuildId,
          ]);
